...
import tools.install.data.Icon;
import tools.install.data.Icons;
import tools.install.data.NDLL;
...
class InstallerBase {
	...
	public function create (buildjs:String, command:String, defines:Hash <String>, includePaths:Array <String>, projectFile:String, target:String, targetFlags:Hash <String>, debug:Bool):Void {
		...
		if (command == "trace") {
			...
			DeviceInstaller.traceEnabled = true;
		...
		if (command == "test" || command == "trace") {
			...
			if (DeviceInstaller.traceEnabled || command == "trace") {
	...
	private function initializeTool ():Void {
      ...
      if (BuildJS.verbose)
	   ...
	   if (useFullClassPaths())
		...
		setDefault ("APP_FILE", "MyApplication.js");
	...
	private function runCommand (path:String, command:String, args:Array <String>):Void {
		...
		try {
			...
			BuildJS.runCommand (path, command, args);
